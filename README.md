...
Так же класс предоставляет набор методов для взаимодействия с этими данными.
- setPaymentAndDelivery(dataForm: Record<string, string>): void - Этот метод устанавливает и способ оплаты, и адрес доставки.
- setContactInfo(dataForm: Record<string, string>): void - Устанавливает электронную почту и номер телефона
...
Конструктор, кроме темплейт контейнера, принимает экземпляр `EventEmitter` для инициации событий.
В полях класса хранятся следующие данные:
...
В метод render принимает стоимость покупок, а так же массив элементов разметки карточек, который отображает в контейнере, за который отвечает и возвращает контейнер.
...
- render(data: HTMLElement): void - заполняет контейнер контентом.

#### Класс ModalWithForm
Родительский класс, объединяющий в себе общие свойства и методы для двух форм. 
При сабмите инициирует событие передавая в объект с данными данные полей ввода формы и запускает открытие следующей формы. Предоставляет методы для отображения ошибок и управления активностью кнопки сабмита.\
...
#### Класс ModaContacts
Расширяет класс ModalWithForm, предназначен для реализации модального окна Contacts.
При инициализации находит необходимые поля в разметке и устанавливает значения свойств. Проводит проверку на заполнение полей. Предоставляет метод для извлечения значений свойств.\
...
Методы:
- getValues(): Record<string, string> - возвращает объект с одержащий в себе значения полей.

#### Класс ModaOrder
Расширяет класс ModalWithForm, предназначен для реализации модального окна Order.
При инициализации находит необходимые поля в разметке и устанавливает значения свойств. Проводит проверку на заполнение полей. Предоставляет метод для извлечения значений свойств.\
...
Поля класса:
- input: HTMLInputElement; - Элемент поля ввода, найденный в DOM
- payment: TPaymentMethod; - значение сбороба оплаты, установленное по клику на соответсвующую кнопку.

Методы:
- getValues(): Record<string, string> - возвращает объект с одержащий в себе значения свойств.
...
- `preview:render` - отрисовыввет превью карточки
- `basket:render` - отрисовывает корзину
- `basket:add` - добавляет продукт в корзину
- `basket:open` - открывает корзину покупок
- `basket:delete` - удаляет продукт из корзины
...
- `modalSuccess:close` - закрывает модальное окно
